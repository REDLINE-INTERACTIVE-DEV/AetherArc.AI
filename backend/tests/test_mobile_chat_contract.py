...
s=p.read_text()
for x in ['backendUrl+"/api/auth/"',"register","login","google","Continue with Google","Continue as Guest","Sign in","Create account","__aetherOAuthToken","openExternal","guest_messages_remaining","Recent chats","Settings","Purple theme","getJson","/api/chat/conversations"]:
    assert x in s, x
