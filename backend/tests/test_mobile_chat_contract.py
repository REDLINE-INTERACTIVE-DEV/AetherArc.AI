...
s=p.read_text()
for x in ['backendUrl+"/api/auth/"',"register","login","Continue with Google","Continue as Guest","Sign in","Create account","__aetherOAuthToken","openExternal","guest_messages_remaining","Recent chats","Settings","Purple theme","getJson","/api/chat/conversations","Thinking…"]:
    assert x in s, x
