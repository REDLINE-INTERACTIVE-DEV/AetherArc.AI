...
def _user_payload(user: User) -> dict:
    return {"id": user.id, "email": user.email, "name": user.name, "avatar_url": user.avatar_url, "provider": user.provider}
...
async def _save_identity(identity: dict, db: AsyncSession) -> User:
    result = await db.execute(select(User).where(User.provider == identity["provider"], User.provider_id == identity["provider_id"]))
    user = result.scalar_one_or_none()
    if user is None:
        result = await db.execute(select(User).where(User.email == identity["email"]))
        user = result.scalar_one_or_none()
    if user is None:
        user = User(email=identity["email"], name=identity.get("name"), avatar_url=identity.get("avatar_url"),
...
async def register(body: Credentials, db: AsyncSession = Depends(get_db)):
    ...
    user = User(email=email, name=email.split("@", 1)[0], provider="email", provider_id=email,
...
def _oauth_url(provider: str) -> str:
    if provider == "google":
    ...
    return ("https://github.com/login/oauth/authorize"
        f"?client_id={settings.GITHUB_CLIENT_ID}&redirect_uri={settings.GITHUB_REDIRECT_URI}&scope=user:email")

@router.get("/google/login")
async def google_login():
    return {"login_url": _oauth_url("google")}

@router.get("/github/login")
async def github_login():
    return {"login_url": _oauth_url("github")}
...
async def exchange_oauth(body: OAuthCodeExchange, db: AsyncSession = Depends(get_db)):
    ...
    return _token_response(await _save_identity(identity, db))
...
async def _verify_google(code: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        token = await client.post("https://oauth2.googleapis.com/token", data={"code":code,"client_id":settings.GOOGLE_CLIENT_ID,
            "client_secret":settings.GOOGLE_CLIENT_SECRET,"redirect_uri":settings.GOOGLE_REDIRECT_URI,"grant_type":"authorization_code"})
        if token.status_code >= 400: raise HTTPException(401, "Google authorization failed.")
        access = token.json().get("access_token")
        if not access: raise HTTPException(401, "Google did not return an access token.")
        info = await client.get("https://openidconnect.googleapis.com/v1/userinfo", headers={"Authorization":f"Bearer {access}"})
        if info.status_code >= 400: raise HTTPException(401, "Could not read Google account.")
        data = info.json()
    return {"provider":"google","provider_id":str(data["sub"]),"email":data["email"],"name":data.get("name"),"avatar_url":data.get("picture")}
...
async def _verify_github(code: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        token = await client.post("https://github.com/login/oauth/access_token", headers={"Accept":"application/json"},
        ...
        if token.status_code >= 400 or not token.json().get("access_token"): raise HTTPException(401, "GitHub authorization failed.")
        access = token.json()["access_token"]; headers={"Authorization":f"Bearer {access}","Accept":"application/vnd.github+json"}
        info = await client.get("https://api.github.com/user",headers=headers)
        if info.status_code >= 400: raise HTTPException(401, "Could not read GitHub account.")
        ...
        if not email: raise HTTPException(401,"GitHub did not provide a verified email.")
    return {"provider":"github","provider_id":str(data["id"]),"email":email,"name":data.get("name") or data.get("login"),"avatar_url":data.get("avatar_url")}

async def _oauth_callback(provider: str, code: str, db: AsyncSession):
    identity = await (_verify_google(code) if provider == "google" else _verify_github(code))
    user = await _save_identity(identity, db)
    token = create_access_token(user.id)
    return RedirectResponse(f"aether://auth/{provider}?access_token={token}", status_code=302)

@router.get("/google/callback")
async def google_callback(code: str, db: AsyncSession = Depends(get_db)):
    return await _oauth_callback("google", code, db)

@router.get("/github/callback")
async def github_callback(code: str, db: AsyncSession = Depends(get_db)):
    return await _oauth_callback("github", code, db)
